...
if (process.env['NODE_ENV'] === 'development') {
  ...
  describe('transactions', function () {
    ...
    it('keyProvider return Promise', function () {
      var eos = Eos.Localnet({ keyProvider: new Promise(function (resolve) {
          resolve(wif);
        }) });
    ...
    it('mockTransactions fail', function () {
      ...
      var eos = Eos.Localnet({ signProvider: signProvider, mockTransactions: 'fail', logger: logger });
    ...
    it('action to unknown contract', function () {
      var logger = { error: null };
      return Eos.Localnet({ signProvider: signProvider, logger: logger }).contract('unknown432').then(function () {
    ...
    it('action to contract', function () {
      return Eos.Localnet({ signProvider: signProvider }).contract('eosio.token').then(function (token) {
