        ...
        options.authorization.forEach(function (auth) {
          if (typeof auth === 'string') {
            var _auth$split = auth.split('@'),
                ...
                _permission = _auth$split2$ === undefined ? 'active' : _auth$split2$;

            authorization.push({ actor: actor, permission: _permission });
  ...
  */
  ...
  function transaction(arg, options, callback) {
    ...
    headers(options.expireInSeconds, checkError(callback, config.logger, function _callee2(rawTx) {
      var feeObj, txObject, buf, tr, transactionId, sigs, chainIdBuf, signBuf;
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              // @TODO 手续费临时解决方案
              _context2.next = 8;
              return _regenerator2.default.awrap(network.getRequiredFee({
                transaction: Fcbuffer.fromBuffer(Transaction, Fcbuffer.toBuffer(Transaction, Transaction.fromObject(Object.assign({ fee: '0.0100 EOS' }, rawTx))))
              }));

            case 8:
              feeObj = _context2.sent;

              assert.equal((0, _typeof3.default)(feeObj.required_fee), 'string', 'expecting required_fee string');
