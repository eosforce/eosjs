  ...
  */
  ...
  function transaction(arg, options, callback) {
    ...
    headers(options.expireInSeconds, checkError(callback, async function(rawTx) {
      ...
      // @TODO 手续费临时解决方案
      const requiredFeeTBufTxObject = Transaction.fromObject(rawTx)
      ...
      const requiredFeeTBuf = Fcbuffer.toBuffer(Transaction, requiredFeeTBufTxObject)

      const requiredFeeTr = Fcbuffer.fromBuffer(Transaction, requiredFeeTBuf)
      const feeObj = await network.getRequiredFee({transaction: requiredFeeTr})
